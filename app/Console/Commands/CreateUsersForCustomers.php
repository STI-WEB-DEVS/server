...
->email) {
 $this
->warn("Skipped customer {$customer
->id}: no email");
 continue;

}
 // Already linked to a user
 $existingLinkedUser = User::where('customer_id', $customer
->id)
->first();
 if ($existingLinkedUser) {
 $this
->info("Already linked: {$customer
->email}");
 continue;

}
 // Same email exists, link it
 $user = User::where('email', $customer
->email)
->first();
 if ($user) {
 $user
...
->id,
 ...
 $this
->info("Linked existing user: {$customer
->email}");
 continue;
